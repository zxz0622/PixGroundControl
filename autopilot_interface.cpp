...
void Autopilot_Interface::start()
{
    ...
    result = (HANDLE)_beginthreadex(NULL,0,start_autopilot_interface_read_thread,this,0,NULL);
    ...
    while (not current_messages.sysid)
        ...
        Sleep(500);							// check at 2Hz  change
        //QTimer::singleShot(100, &eventloop, SLOT(quit()));//如果接受信息用信号槽的话用这段代码延时
        //eventloop.exec();
        }
...
/*
    ...
    result = (HANDLE)_beginthreadex(NULL,0,start_autopilot_interface_write_thread,this,0,NULL);
...
void Autopilot_Interface::read_messages()
{
    ...
    while (!received_all and ! time_to_exit)
    {
        ...
        if (success)
            ...
            switch (message.msgid)
                ...
                case MAVLINK_MSG_ID_HEARTBEAT:
                        ...
                        mavlink_msg_heartbeat_decode(&message, & (current_messages.heartbeat));//数据存储在current_messages.heartbeat中
        ...
        if (writing_status > false)
            {
                Sleep(1);							// look for components of batches at 10kHz，changed应该是用usleep（100）
            }
        Sleep(1);
...
void Autopilot_Interface::write_thread(void)
{
    ...
    sp.type_mask		=
         MAVLINK_MSG_SET_POSITION_TARGET_LOCAL_NED_VELOCITY & MAVLINK_MSG_SET_POSITION_TARGET_LOCAL_NED_YAW_RATE;
    sp.coordinate_frame = MAV_FRAME_LOCAL_NED;
    sp.vx				= 0.0;
    sp.vy				= 0.0;
    sp.vz				= 0.0;
...
unsigned int __stdcall start_autopilot_interface_read_thread(void * args)
{
    // takes an autopilot object argument
    Autopilot_Interface * autopilot_interface = (Autopilot_Interface *)
...
unsigned int __stdcall start_autopilot_interface_write_thread(void * args)
{
    // takes an autopilot object argument
    Autopilot_Interface * autopilot_interface = (Autopilot_Interface *)
